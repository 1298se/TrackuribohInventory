...
def find_condition_by_name(session: Session, name: str) -> Optional[Condition]:
    ...
    return session.scalar(select(Condition).where(Condition.name == normalized_name))
...
def find_printing_by_name(session: Session, name: str) -> Optional[Printing]:
    ...
    normalized_name = " ".join(word.capitalize() for word in name.split())
    return session.scalar(select(Printing).where(Printing.name == normalized_name))
            ...
            for order in current_chunk:
                try:
                    ...
                    with session.begin():
                        ...
                        is_canceled = (
                            order.get("shippingStatus", "").lower() == "canceled"
                        )

                        if is_canceled:
                            print(
                                f"Skipping order {order.get('vendorId')}: Order was canceled (full refund)"
                            )
                            skipped_orders += 1
                            continue
                        ...
                        currency = (
                            order.get("products", [{}])[0].get("currency", "USD")
                        ...
                        for product_data in order.get("products", []):
                            ...
                            if not product_tcgplayer_id:
                                print(
                                    f"Skipping product in order {order.get('vendorId')}: No product ID"
                                )
                                continue

                            # Find product
                            product = find_product_by_tcgplayer_id(
                                session, product_tcgplayer_id
                            )
                            if not product:
                                print(
                                    f"Skipping product {product_tcgplayer_id}: Not found in database"
                                )
                                continue
                            ...
                            if not condition_name:
                                print(
                                    f"Skipping product {product_tcgplayer_id}: Missing condition field"
                                )
                                continue
                            condition = find_condition_by_name(session, condition_name)
                            if not condition:
                                print(
                                    f"Skipping product {product_tcgplayer_id}: Condition '{condition_name}' not found"
                                )
                                continue
                            ...
                            if not printing_name:
                                print(
                                    f"Skipping product {product_tcgplayer_id}: Missing finish field"
                                )
                                continue
                            printing = find_printing_by_name(session, printing_name)
                            if not printing:
                                print(
                                    f"Skipping product {product_tcgplayer_id}: Printing '{printing_name}' not found"
                                )
                                continue
                            ...
                            if not sku:
                                print(
                                    f"Skipping product {product_tcgplayer_id}: SKU not found"
                                )
                                continue
                            ...
                            line_item_data: LineItemData = LineItemData(
                                sku_id=sku.id,
                                quantity=quantity,
                        ...
                        if valid_order and line_items_data:
                            ...
                            print(
                                f"Successfully imported order {order.get('vendorId')}"
                            )
                        else:
                            print(
                                f"Skipping order {order.get('vendorId')}: No valid products"
                            )
