...
async def update_inventory_price_data():
    ...
    async with TCGPlayerCatalogService() as service:
        with SessionLocal() as session:
            all_sku_ids = [sku.id for (sku, _, _, _) in session.execute(query_inventory_items()).all()]

            sku_id_batches = [all_sku_ids[i:i + PARTITION_SIZE] for i in range(0, len(all_sku_ids), PARTITION_SIZE)]

            for batch_sku_ids in sku_id_batches:
                await update_latest_sku_prices(
                    session=session,
                    catalog_service=service,
                    sku_ids=batch_sku_ids
                )
