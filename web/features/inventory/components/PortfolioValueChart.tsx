...
import { EmptyState } from "@/shadcn/ui/empty-state";
import { Alert, AlertDescription } from "@/shadcn/ui/alert";
import { TrendingUp, TrendingDown } from "lucide-react";
import { cn } from "@/lib/utils";
import { Skeleton } from "@/shadcn/ui/skeleton";
import {
  useGetInventoryPerformanceQuery,
...
export function PortfolioValueChart({
  ...
  const { data: currentMetrics, isLoading: metricsLoading } = useQuery(
    useGetInventoryMetricsQuery(catalogId)
  );
  ...
  } = useQuery(useGetInventoryPerformanceQuery(catalogId, daysNumber));
  ...
  const portfolioChange = useMemo(() => {
    ...
    const absoluteChange = currentValue.amount - previousValue;
              ...
              <>
                ...
                <div className="flex items-baseline gap-4">
                  <h2 className="text-3xl font-bold tracking-tight">
                    {formatCurrency(
                      currentMetrics?.total_market_value.amount ?? 0
                    )}
                  </h2>
                  {portfolioChange && (
                    <div
