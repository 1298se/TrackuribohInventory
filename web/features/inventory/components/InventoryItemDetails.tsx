...
import { formatSKU } from "@/features/inventorySearch/utils";
import { ProductHeader } from "@/features/inventorySearch/components/ProductHeader";
import { PriceHistoryChart } from "./PriceHistoryChart";
import { TimeRangeToggle } from "@/shadcn/ui/time-range-toggle";
import { Select } from "@/features/inventorySearch/components/MarketplaceSelector";
import {
  useGetInventoryItemQuery,
  useGetInventoryTransactionsQuery,
  useGetInventoryPriceHistoryQuery,
  useGetSkuMarketplacesQuery,
} from "../api";
import { useQuery } from "@tanstack/react-query";
import { InventorySKUTransactionLineItem } from "../types";
// import { MarketDepthWithMetrics } from "@/features/inventorySearch/components/MarketDepthWithMetrics";
// import { useSkuMarketData } from "@/features/inventorySearch/api";
...
export function InventoryItemDetails({
  ...
  const {
    ...
    error: itemError,
  } = useQuery(useGetInventoryItemQuery(inventoryItemId));

  const {
    data: transactionsData,
    isLoading: transactionsLoading,
    error: transactionsError,
  } = useQuery(useGetInventoryTransactionsQuery(inventoryItemId));

  // Commented out market data as it depends on useSkuMarketData which may not exist yet
  // const {
  ...
  } = useQuery(useGetSkuMarketplacesQuery(inventoryItem?.sku.id || null));

  const {
    data: priceHistoryData,
    isLoading: priceHistoryLoading,
    error: priceHistoryError,
  } = useQuery(useGetInventoryPriceHistoryQuery(
        ...
        <TabsContent value="market" className="mt-6">
          <Card className="overflow-hidden">
            ...
            <CardContent className="p-0 space-y-6">
              ...
              {/* Market Depth Section - Commented out until dependency is available */}
