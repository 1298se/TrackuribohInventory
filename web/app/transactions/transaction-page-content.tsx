...
export function TransactionsPageContent({ token }: { token: string }) {
  ...
  const { data, isLoading, mutate, error } = useTransactions(token, filters);
  // Fetch filter options for dropdowns
  const { data: filterOptions } = useTransactionFilterOptions(token);
  // Fetch aggregate metrics
  const { data: metricData, isLoading: metricLoading } =
    useTransactionMetrics(token);
  // Hook to delete transactions
  const deleteMutation = useDeleteTransactions(token);
