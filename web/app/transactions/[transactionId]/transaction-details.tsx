...
import { SelectProductDialog } from "../../legacyInventory/_util/select-product-dialog";
...
} from "@/app/catalog/_components/schemas";
import { formatSKU } from "@/app/catalog/_components/utils";
...
export function TransactionDetails({ transactionId }: TransactionDetailsProps) {
  const {
    ...
    error,
    mutate,
  } = useTransaction(transactionId, "test");
  const { trigger: updateTransaction, isMutating } =
    useUpdateTransaction("test");
  const { data: platforms, isLoading: isPlatformsLoading } =
    usePlatforms("test");
  const { trigger: calculatePrices, isMutating: isCalculating } =
    useCalculateWeightedPrices("test");
