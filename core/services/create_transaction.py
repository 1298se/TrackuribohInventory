...
async def calculate_weighted_unit_prices(
    ...
    line_items_by_tcgplayer_id = {
        sku_id_to_tcgplayer_id[item.sku_id]: item for item in line_items
    }
    ...
    for sku_price in sku_prices.results:
        ...
        input_item = line_items_by_tcgplayer_id.get(
            sku_price.sku_id
        )  # This uses the mapping we already had
    ...
    total_priced_items_units = (
        sum(item.quantity for item, _ in items_with_prices) if items_with_prices else 0
    )
    total_unpriced_items_units = (
        sum(item.quantity for item in items_without_prices)
    ...
    priced_allocation = (
        (total_priced_items_units / total_units) if total_units > 0 else Decimal(0)
    )
    ...
    ratio_for_priced_items = (
        amount_for_priced_items / priced_items_total
        if priced_items_total > 0
        else Decimal(0)
    )
    unpriced_unit_price = (
        amount_for_unpriced_items / total_unpriced_items_units
        if total_unpriced_items_units > 0
        else Decimal(0)
    )
    ...
    for item, sku_price in items_with_prices:
        ...
        line_items_data.append(
            LineItemData(
                ...
                unit_price_amount=tcgplayer_id_to_lowest_price.get(
