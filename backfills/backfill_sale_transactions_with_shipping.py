...
def create_transaction_request(
    ...
    line_items = [
        LineItemCreateRequestSchema(
            sku_id=line_item.sku_id, quantity=line_item.quantity
        )
...
async def process_single_transaction(
    transaction: Transaction, tcgplayer_service: TCGPlayerCatalogService, session
):
    """
    ...
    try:
        ...
        logger.info(
            f"Processing transaction {transaction_id} - Counterparty: {transaction.counterparty_name}"
        )
        logger.info(
            f"  Shipping Cost: {transaction.shipping_cost_amount}, Line Items: {len(transaction.line_items)}"
        )
        logger.info(
            f"  SKU IDs: {[line_item.sku_id for line_item in transaction.line_items]}"
        )
        ...
        if has_missing_tcgplayer_ids:
            logger.warning(
                f"  Transaction {transaction_id} has line items with missing TCGPlayer IDs"
            )
        ...
        new_transaction = await create_transaction_service(
            request, tcgplayer_service, session
        )

        # Log success
        logger.info(
            f"✓ Successfully recreated transaction. Old ID: {transaction_id}, New ID: {new_transaction.id}"
        )
        ...
        logger.error(
            f"✗ Error processing transaction {transaction_id}: {type(e).__name__}: {e}"
        )
        logger.error(f"  Traceback: {traceback.format_exc()}")
        return False


async def recreate_transactions(
    transactions: list[Transaction], tcgplayer_service: TCGPlayerCatalogService, session
):
    """
    ...
    for transaction in transactions:
        ...
        result = await process_single_transaction(
            transaction, tcgplayer_service, session
        )
...
async def main():
    ...
    try:
        ...
        with SessionLocal() as session:
            ...
            logger.info(
                f"Found {len(transactions)} SALE transactions with shipping costs"
            )
